...
// or null otherwise.
    ...
    public T remove() 
    {
        ...
        //select a no from 0 to numberOfEntries
        int x=rn.nextInt(numberOfEntries);
        
        //x varies from 0 to (numberOfEntries-1)
        x=x%numberOfEntries;
        
        //if x is zero then we need to remove the first element
        //if x is not zero shift the xth element to the first position
        if(x!=0)
        {
        	...
        	//shifting the xth node to the first position
        	prev.next=curr.next;
        	curr.next=firstNode;
        	firstNode=curr;
        }
        
        //already written to remove first node
        if (firstNode != null) {
    ...
    public boolean equals(LinkedBag<T> aBag) 
    {
        ...
        //if no of entries are not equal then bags are not equal 
        if(this.numberOfEntries != aBag.numberOfEntries)
        	return result;
        
        //node variable initialized to first node of bag
        Node scout=this.firstNode;
        for(int i=0;i< numberOfEntries;i++)
        {
        	//checking the frequency of each element in both the bags
        	int f1=this.getFrequencyOf(scout.data);
        	int f2=aBag.getFrequencyOf(scout.data);
        	//if frequency in both bag is different than return false (bag not equal)
        	if(f1!=f2)
        		return result;
        	
        	scout=scout.next;
        }
        //if loop ends means all elements have same frequency in both bags so return true
        result=true;
        return result;
    }  // end equals
    ...
    public boolean duplicateAll() 
    {
        ...
        //store no of entries in variable x
    ...
    public void removeDuplicates() 
    {
    	...
    	//x is initialized to first node
        Node x=this.firstNode;
        LinkedBag<T>newFirst=new LinkedBag<T>();
        
        //traverse the whole bag
        while(x!=null)
        {
        	//if the frequency of the selected data item is 0 then add this data item to new linkedbag
        ...
        //replacing the original bag with the new linkedbag with no duplicate in it 
